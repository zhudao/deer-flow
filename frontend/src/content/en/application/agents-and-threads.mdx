...
How thread state is represented in that backend — full-value checkpoints or
LangGraph's `DeltaChannel` — is a separate switch,
`database.checkpoint_channel_mode`. See
[Checkpoints and Delta Channels](/docs/harness/checkpoints).
