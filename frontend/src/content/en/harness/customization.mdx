...
class RedisMemoryStorage(MemoryStorage):
    # `create_storage()` builds the class as `cls(config)`, so it must take the backend config.
    def __init__(self, config) -> None:
        ...

    # The memory updater calls these with `user_id=` — and `save()` also with
    # `expected_revision=` — so both extra parameters must stay in the signature.
...
Unlike `sandbox.use:` above, `storage_class` is resolved with a dotted `module.Class` path — `RedisMemoryStorage` is split off after the last dot, so a `module:Class` value fails to import.
