...
class RedisMemoryStorage(MemoryStorage):
    # `create_storage()` 以 `cls(config)` 构造存储类，因此必须接收 backend config。
    def __init__(self, config) -> None:
        ...

    # memory updater 调用这些方法时始终带上 `user_id=`，`save()` 还会带上
    # `expected_revision=`，所以这两个参数必须保留在签名里。
...
与上面的 `sandbox.use:` 不同，`storage_class` 按 `module.Class` 点号路径解析——类名取自最后一个点号之后，所以写成 `module:Class` 会导入失败。
