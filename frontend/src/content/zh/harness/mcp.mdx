...
MCP 工具缓存追踪 `extensions_config.json` 的解析路径和 `(mtime, size, sha256)` 内容签名。签名变化只是触发重新比对的信号：缓存会比较已启用服务器的解析后配置与全局 `mcpInterceptors` 列表，只有该有效切片真正变化时才会淘汰已缓存的工具和会话池。

因此仅修改 Skills 或 middleware 不会重建工具或关闭会话；切换到另一份 MCP 配置完全相同的配置文件同样保留会话。启用、禁用或修改某个服务器会在下次请求时重新加载工具。当没有启用任何 MCP 服务器时，工具组装会回收此前已初始化的缓存与会话，但不会发起新的工具发现。

Gateway 的显式 **重置 MCP 工具缓存**（`POST /api/mcp/cache/reset`）与此自动检查相互独立，始终清理缓存和会话池。
